...
internal static partial class Parser
{
    ...
    {
        string liveId = TryExtractLiveId(raw) ?? throw new Exception("Live Stream canonical link not found; Video ID not found in page source");
    ...
    private static string? TryExtractLiveId(string raw)
    {
        ...
        Match ogUrlResult = LiveIdOgUrlRegex().Match(raw);
        if (ogUrlResult.Success)
        {
            return ogUrlResult.Groups[1].Value;
        }

        Match twitterUrlResult = LiveIdTwitterUrlRegex().Match(raw);
        if (twitterUrlResult.Success)
        {
            return twitterUrlResult.Groups[1].Value;
        }

        Match shortlinkResult = LiveIdShortlinkRegex().Match(raw);
        if (shortlinkResult.Success)
        {
            return shortlinkResult.Groups[1].Value;
        }

        Match fallbackIdResult = LiveIdFallbackRegex().Match(raw);
        if (fallbackIdResult.Success)
        {
            return fallbackIdResult.Groups[1].Value;
        }

        Match genericIdResult = LiveIdGenericRegex().Match(raw);
        if (genericIdResult.Success)
        {
            return genericIdResult.Groups[1].Value;
        }
    ...
    [GeneratedRegex(
        "<meta property=\"og:url\" content=\"https:\\/\\/www\\.youtube\\.com\\/watch\\?v=([^\"]+)\">",
        RegexOptions.Compiled
    )]
    private static partial Regex LiveIdOgUrlRegex();

    [GeneratedRegex(
        "<meta name=\"twitter:url\" content=\"https:\\/\\/www\\.youtube\\.com\\/watch\\?v=([^\"]+)\">",
        RegexOptions.Compiled
    )]
    private static partial Regex LiveIdTwitterUrlRegex();

    [GeneratedRegex(
        "<link rel=\"shortlink\" href=\"https:\\/\\/youtu\\.be\\/([^\"]+)\">",
        RegexOptions.Compiled
    )]
    private static partial Regex LiveIdShortlinkRegex();

    [GeneratedRegex(
        "\"liveStreamabilityRenderer\":\\s*{\\s*\"videoId\":\\s*\"([^\"]+)\"",
        RegexOptions.Compiled
    )]
    private static partial Regex LiveIdFallbackRegex();

    [GeneratedRegex(
        "\"videoId\":\\s*\"([a-zA-Z0-9_-]{11})\"",
        RegexOptions.Compiled
    )]
    private static partial Regex LiveIdGenericRegex();
    ...
    private static readonly Regex _liveIdOgUrlRegex = new(
        "<meta property=\"og:url\" content=\"https:\\/\\/www\\.youtube\\.com\\/watch\\?v=([^\"]+)\">",
        RegexOptions.Compiled
    );

    private static Regex LiveIdOgUrlRegex() => _liveIdOgUrlRegex;

    private static readonly Regex _liveIdTwitterUrlRegex = new(
        "<meta name=\"twitter:url\" content=\"https:\\/\\/www\\.youtube\\.com\\/watch\\?v=([^\"]+)\">",
        RegexOptions.Compiled
    );

    private static Regex LiveIdTwitterUrlRegex() => _liveIdTwitterUrlRegex;

    private static readonly Regex _liveIdShortlinkRegex = new(
        "<link rel=\"shortlink\" href=\"https:\\/\\/youtu\\.be\\/([^\"]+)\">",
        RegexOptions.Compiled
    );

    private static Regex LiveIdShortlinkRegex() => _liveIdShortlinkRegex;

    private static readonly Regex _liveIdFallbackRegex = new(
        "\"liveStreamabilityRenderer\":\\s*{\\s*\"videoId\":\\s*\"([^\"]+)\"",
        RegexOptions.Compiled
    );

    private static Regex LiveIdFallbackRegex() => _liveIdFallbackRegex;

    private static readonly Regex _liveIdGenericRegex = new(
        "\"videoId\":\\s*\"([a-zA-Z0-9_-]{11})\"",
        RegexOptions.Compiled
    );

    private static Regex LiveIdGenericRegex() => _liveIdGenericRegex;
